...
class Order {
    ...
    public function add() {
        ...
        $str_order_date = date('Y-m-d', $this->orderDate);
    ...
    public static function loadAll(){
        ...
        $sql = "select * from orders o left join users u on u.Id = o.user";
        ...
        while ($row = mysqli_fetch_array($list)) {
            ...
            $user = $row["user"];
            $total = $row["total"];
            if($row["hoTen"] == '' || $row["hoTen"] == null){
                $hoten = $row["Username"];
            }
            else if( $row["sdt"]  == '' || $row["sdt"] == null ){
                $sdt = 0;
            }
