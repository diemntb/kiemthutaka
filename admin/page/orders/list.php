                    ...
                    <?php if (isset($list)) { ?>
                        <?php foreach($list as $item) { ?>
                            <tr >
                                ...
                                <td><?php if($item->userID != null || $item->userID != '') echo $item->userID; else echo $item->hoten; ?></td>
                                <td><?php if($item->email != null || $item->email != '') echo $item->email; else echo $item->email; ?></td>
                                <td><?php if($item->sdt != null || $item->sdt != '') echo $item->sdt; else echo $item->sdt; ?></td>
                                <td> <?php echo number_format($item->total); ?></td>
                                <td><?php echo $item->orderDate; ?></td>
                                <!--<td><?php echo $item->note; ?></td>-->
                                <td> <?php
                                if( $item->status == 0 ){
                                    echo '<span style="color: darkorange; font-weight: bold">Chờ xử lý</span>';
                                }
                                else if( $item->status == 1 ){
                                    echo '<span style="color: springgreen">Đã xử lý</span>';
                                }
                                else if( $item->status == 2 ){
                                    echo '<span style="color: darkred">Hủy đơn hàng</span>';
                                }
                                ?></td>
